...
图 1-1。The installation dialog for Firebug
...
图 1-2。The Firebug add-on opens to the console panel
...
Visit [`www.apachefriends.org/en/xampp.html`](http://www.apachefriends.org/en/xampp.html)，并下载适用于您操作系统的最新版本 XAMPP（截至本文撰写时为 7.0.1）。本书将假设使用 PHP 7.x 版本；这从第 3 章开始将变得重要。打开下载的文件。对于 PC，运行 EXE 文件，选择一个目录并安装。对于 Mac，挂载 DMG，然后将 XAMPP 文件夹拖到您的 `Applications` 文件夹中。打开 XAMPP 文件夹中的 XAMPP 控制面板，并启动 Apache。导航到 `http://localhost/` 以确保 XAMPP 正常工作。如果正常，XAMPP 主页会通知您。
