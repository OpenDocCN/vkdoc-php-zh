...
`database/migrations/2014_07_11_185334_create_users_table.php`
...
客户机利用构造函数中传递的`Migrator`并最终运行迁移器。上面要注意的重点是`$this->migrator->run($paths = [], array $options = [])`方法。这就是`Invoker`的开始。如果您想了解迁移是如何回滚的，您可以查看另一个名为`RollbackCommand`的命令。它像`MigrateCommand`一样使用`Migrator`来调用`rollback`方法，而不是`run`。
