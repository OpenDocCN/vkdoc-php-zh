...
function dgd7glue_node_view($node, $view_mode, $langcode) {
  ...
  if ($node->type == 'suggestion' && $view_mode == 'full') {
    ...
    $next = db_query('SELECT title, nid FROM {node} WHERE nid > :nid AND status =![images](img/U002.jpg)
...
```
$result = db_query_range('SELECT f.*, u.uid as user_uid FROM {users} u INNER JOIN![images](img/U002.jpg)
 {file_managed} f ON u.picture = f.fid WHERE u.picture <> 0 AND u.uid > :uid ORDER BY![images](img/U002.jpg)
 u.uid', 0, $limit, array(':uid' => $sandbox['last_uid_processed']))->fetchAllAssoc('fid',![images](img/U002.jpg)
 PDO::FETCH_ASSOC);
```
...
function dgd7glue_nextprev_suggestion($nid, $previous = FALSE) {
  ...
  return db_query_range("SELECT title, nid FROM {node} WHERE nid $co :nid AND type =![images](img/U002.jpg)
 :type AND status = :status ORDER BY nid $direction", 0, 1, array(':nid' => $nid, ':type' =>![images](img/U002.jpg)
...
function dgd7glue_format_link($link) {
  return l($link['text'], 'node/' . $link['nid'], array('attributes' => array('title' =>![images](img/U002.jpg)
 $link['title'])));
}
```
