...
CouchDB 可以接受 HTTP 命令，当然也可以使用`curl -X PUT [`localhost:5984/dbname`](http://localhost:5984/dbname)`命令创建数据库；使用名为 Futon 的 CouchDB 管理界面要舒服得多。可以使用您最喜欢的网络浏览器并将其指向`[`localhost:5984/_utils`](http://localhost:5984/_utils)`来访问该界面。如果服务器不在本地主机上，您应该替换为服务器名称和端口。它是可配置的。在 Opera 中，结果看起来像图 7-2 。
