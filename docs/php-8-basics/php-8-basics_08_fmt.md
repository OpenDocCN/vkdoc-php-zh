...
- `array` — 第一个数组。
- `arrays` — 用于比较的数组。
- `value_compare_func` — 回调比较函数。如果第一个参数被认为分别小于、等于或大于第二个参数，则比较函数必须返回一个小于、等于或大于零的整数。
...
- `array` — 第一个数组。
- `arrays` — 用于比较的数组。
- `value_compare_func` — 如果第一个参数被认为分别小于、等于或大于第二个参数，则比较函数必须返回一个小于、等于或大于零的整数。
...
- `array` — 输入数组。
- `flags` — 可选的第二个参数 `flags` 可用于通过以下值修改排序行为。
...
- `array` — 输入的数组。
- `callback` — 通常，`callback` 接受两个参数：数组参数的值和键名/索引。
- `arg` — 如果提供了可选的 `arg` 参数，它将被作为第三个参数传递给回调函数。
...
“Massimo” is in the text and the `i` means case-insensitive, which means “Massimo” and “Massimo” are the same. Removing the `i` from the pattern means a match will be not found because regular expressions are by default case-sensitive.
