...
使用表 10-1 中的属性，并为将接受排序规则的列选择排序规则 utf8 _ general _ ci(reg _ date 和 member_level 没有排序规则)。创建列时，将出现 member_id 和 user_name 索引的弹出对话框。对于 member_id 弹出窗口，这两个字段都将是主字段。在用户名索引弹出窗口中，在第一个字段中输入**用户名**，在第二个字段中输入**唯一用户名**。
