...
1.  验证表单是否使用正确的操作提交。
2.  使用`htmlentities()`清理用户输入。
3.  从数据库中检索具有匹配用户名的用户数据。
4.  将用户信息存储在变量`$user`中，并确保它不为空。
5.  根据用户提供的密码和数据库中存储的密码生成加盐散列。
6.  确保散列匹配。
7.  使用数组将用户数据存储在当前会话中，并返回`TRUE`。

> **注意**
> 加盐散列将在下一节“构建创建加盐散列的方法”中讨论。

首先在`Admin`类中定义方法，并使用下面的代码完成前面的步骤 1 和 2:
