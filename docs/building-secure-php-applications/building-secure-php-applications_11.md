...
**第 3 步：为 HTTPS 配置 Laravel**

在你的 Laravel 应用中，你需要对其进行配置，使其与 HTTPS 无缝协作。使用以下设置更新你的 `.env` 文件：

```env
APP_URL=https://your-domain.com
```

**第 4 步：在 Laravel 中间件中启用 HTTPS**

创建一个中间件来强制使用 HTTPS。运行以下命令生成一个新的中间件：
...
在 `config/app.php` 中，确保将 `url` 配置设置为使用 HTTPS：

```php
'url' => env('APP_URL', 'https://your-domain.com'),
```

**第 6 步：HSTS（HTTP 严格传输安全）**

为了增强安全性，你可以在 Laravel 应用中启用 HTTP 严格传输安全（HSTS）。将以下中间件添加到 `App\Http\Kernel` 中的 `$middleware` 数组：
