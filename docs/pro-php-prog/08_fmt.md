...
对于每一列，都有一个描述——一个辅助类 `stdClass` 的对象。该对象通过使用 `$res` 对象的 `fetch_field` 方法来检索。以下是相关片段：
...
`name` 属性显然指的是列名。在处理视图时，`orgname` 和 `orgtable` 很重要。SQL 标准描述了称为“视图”的对象，这些对象本质上是命名查询。允许查询重命名列，因此新名称将在 `name` 属性中，而原始名称和表将在 `orgname` 和 `orgtable` 属性中。除了名称和长度列之外，最重要的列是 `type` 列。不幸的是，MySQLi 文档中没有记录这些类型的含义。然而，根据经验，我们知道 3 是整数，5 是双精度数，253 是可变字符，7 是时间戳数据类型。
...
所有现代搜索引擎都有这样的能力。当然，不止一种文本搜索软件，既有开源的，也有商业的。开源文本引擎有 `Sphinx`、`Lucene`、`Xapian` 和 `Tsearch2`，它们各有优缺点。还有一些商业产品，如 `Oracle*Text` 或 `Autonomy Corp` 的 `IDOL engine`。本章的其余部分将专门介绍 `Sphinx`，这是一个由 `Sphinx Technologies` 开发的开源文本搜索引擎。公司网址为：`sphinxsearch.com`。
...
> **注意**：`Sphinx` 数据源中查询的第一列必须是主键。主键也必须是整数。支持大的 64 位整数。
...
> **注意**：由几行组成的字段，比如我们的 SQL 字段，必须像前面的例子一样使用反斜杠字符。
