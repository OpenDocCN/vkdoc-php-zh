...
保存后，您可以在浏览器中重新加载`http://localhost/blog/test.php`。你应该看到`local values for upload_max_filesize`现在是 1M（意思是 1MB）。`Master Values`仍然是 128M，但是对于你的博客文件夹和子文件夹，它现在改为 1M。
...
你知道`$_FILES`超级全局天生支持七种不同的错误。您只是在测试其中一个问题：上传文件太大。
...
`<a href='admin.php?page=images&amp;delete-image=img/coffee.jpg'>delete</a>`
