...
当`register_globals`被启用并且 URL 被设置为`foobar.com/login.php?is_admin=true`时，清单 11-1 中的脚本将总是被授予管理员权限。
...
大多数读者都知道，用 HTTP GET 方法提交的表单域可以通过直接修改 URL 查询来修改。这通常是期望的行为。例如，`stackoverflow.com`的搜索表单可以单独使用查询来提交。见清单 11-4 。
...
track_errors = Off      //keeps track of last error inside global $php_errormsg. We do not
                        //want this.
