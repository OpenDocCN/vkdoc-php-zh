...
2.  在[出版社的网页上下载第](http://apress.com) 3 章的 PHP 文件。并将它们放入 XAMPP *htdocs* 或 easyPHP *eds-www* 的新登录文件夹中。或者，您可以在阅读本章时，通过在 HTML 编辑器中键入每个文件来练习创建 PHP 和 HTML 清单。将它们保存在您的*登录*文件夹中，以便您可以运行它们。
...
在运行 XAMPP 或 easyPHP 的情况下，在浏览器中输入**http://localhost/log in/index . PHP**查看带有新登录按钮的主页，如图 3-1 所示。
...
清单 3-5 显示了修改后的 PHP 代码，用于过滤 process-register-page.php 的*条目。*
