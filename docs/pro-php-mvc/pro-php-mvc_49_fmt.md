...
我们应该在尽可能多的类中添加类似的方法，例如表 19-1 中所列的事件。
...
| `framework.view.construct.after` | `function($file, $template)` <br/> `$file` — 应渲染的模板文件 <br/> `$template` — 被实例化的模板 |
| `framework.view.render.before` | `function($file)` <br/> `$file` — 应被渲染的模板文件 |
...
$this->actionView->set("password_error", "Email address and/or password is incorrect");
