...
`Facade`真的是一个很简单的概念。这只是为层或子系统创建一个单一入口点的问题。这有许多好处。它有助于将项目中不同的区域相互分离。对于客户端编码人员来说，使用简单的方法来实现清晰的目标是非常有用和方便的。它通过将子系统的使用集中在一个地方来减少错误；对子系统的更改应该会导致可预测位置的故障。复杂子系统中的`Facade`类也可以最小化错误，否则客户端代码可能会错误地使用内部函数。
