...
本章中的教程基于第二章中创建的页面。我们将通过为数据库和包含 PHP 和 HTML 页面的文件夹使用一个新名称来阐明文件结构。这是必要的，因为我们发现，如果学生一次又一次地修改，他们会陷入可怕的混乱。因此，作为一般规则，我们将继续为本书的每个新章节使用单独的文件夹和新的数据库名称。但是，一个表可以和前面的教程同名(例如，`users`)。如果表在不同的数据库中，这是可以接受的。

我们开始吧。创建一个新文件夹和一个新数据库，如下所示:

1.  在 XAMPP `htdocs` 或 easyPHP `eds-www` 文件夹中，新建一个名为 `login` 的文件夹。

2.  在[出版社的网页上下载第](http://apress.com) 3 章的 PHP 文件。并将它们放入 XAMPP `htdocs` 或 easyPHP `eds-www` 的新登录文件夹中。或者，您可以在阅读本章时，通过在 HTML 编辑器中键入每个文件来练习创建 PHP 和 HTML 清单。将它们保存在您的`login`文件夹中，以便您可以运行它们。
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `user_id` | `MEDIUMINT` | 6 | 没有人 | `UNSIGNED` | -是吗 | `PRIMARY` | ·······················。 |
| `first_name` | `VARCHAR` | 30 | 没有人 |   | -是吗 |   | -是吗 |
| `last_name` | `VARCHAR` | 40 | 没有人 |   | -是吗 |   | -是吗 |
| `email` | `VARCHAR` | 50 | 没有人 |   | -是吗 |   | -是吗 |
| `password` | `CHAR` | 60 | 没有人 |   | -是吗 |   | -是吗 |
...
在运行 XAMPP 或 easyPHP 的情况下，在浏览器中输入`http://localhost/login/index.php`查看带有新登录按钮的主页，如图 3-1 所示。
...
多余的按钮被两个有意义的链接取代，即删除条目和取消。“删除条目”按钮重新加载注册页面，然后显示空字段。在与不熟悉电脑的用户测试网站后，我们选择了 Er *作为条目*,而不是清除或删除所有条目。他们被“清除”和“擦除所有”的措辞弄糊涂了，但他们立即明白了“擦除”条目。取消按钮使用户返回主页。清单 3-2a 给出了注册和新密码页面的修改标题代码。
...
新文件被命名为`members-header.php`。新的注销按钮有一个链接，将用户带到名为`logout.php`的页面。这反过来会将用户发送到主页。为什么链接不直接到主页？因为`logout.php`的中间页面包含了一些在访问主页之前关闭会话的代码。会话是一种设备，将在下一节中解释。新标题的注销链接显示在下面的代码片段中。
...
清单 3-3 创建了成员页面的新标题 (`members-header.php`)。
...
清单 3-4a 创建了修订后的“谢谢”页面标题的代码 (`thanks-header.php`)。
...
Listing 3-4b Creating the Revised "Thank You" Page (`register-thanks.php`)
...
Listing 3-5 Creating the Amended Registration Page (`process-register-page.php`)
...
[`www.php.net/manual/en/function.filter-input.php`](http://www.php.net/manual/en/function.filter-input.php)
...
访问 `phpMyAdmin` 并单击数据库 `logindb`。然后单击用户表。单击“结构”选项卡。查看记录下方，在*加 1 列*字样旁找到加符号，如图 3-6 所示。
...
**Listing 3-7a** Creating the Login Page (`login.php`)
...
单词“三元”( trivial )与一个海鸥科海鸟保护区没有任何关系。它是程序员和网站开发者使用的数学术语。它是“一元、二元、三元”系列中的第三个运算符换句话说，*三元*有三个部分。三元运算符通常被称为*三元条件运算符*，因为它是设置条件(if-then 表达式)的简洁方式。操作员使用符号？还有:。我们使用的格式将用户重定向到管理页面的 URL 或成员页面的 URL。代码如下:
...
从[出版社下载`members-page.php`文件。或者使用清单](http://apress.com) 3-8 手工编码来创建页面。图 3-12 显示了为本教程创建的成员页面。
...
PHP `session_start()` 语句必须立即出现在代码的顶部(就在所有的私有页面都将以 `session_start()` 函数开始。`header()` 重定向语句确保如果没有启动会话或者 `user_level` 不为零，用户将使用 `header` 命令重定向到登录页面。这个命令创建一个 HTTP Get 消息来检索登录页面。`exit()` 函数还通过立即结束页面的执行来增加安全性。这确保了除非用户正确登录，否则不允许对页面的其余部分进行访问。
...
清单 3-9a 创建管理页面标题 (`admin-header.php`)
