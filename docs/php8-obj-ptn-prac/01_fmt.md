...
申请很乱。他们可能需要将文件安装在非标准位置，或者想要设置数据库，或者需要修补服务器配置。简而言之，应用需要在安装过程中完成`填充`。`Phing` 是一个名为 `Ant` 的 `Java` 工具的忠实端口。`Phing` 和 `Ant` 解释一个构建文件，并按照您告诉他们的任何方式处理您的源文件。这通常意味着将它们从源目录复制到系统中的各个目标位置，但是，随着您的需求变得更加复杂，`Phing` 可以毫不费力地进行扩展来满足它们。
