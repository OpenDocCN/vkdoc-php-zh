...
PDO 的代码如下（完整代码见 `ch17` 文件夹中的 `register_2way_pdo.php`）：
...
严格来说，没必要为 `$key` 使用绑定参数，因为它并非来自用户输入。但如果直接将其嵌入查询中，整个查询需要用双引号包裹，而 `$key` 则需要用单引号包裹。

要测试上述脚本，只需将它们包含在 `register_db.php` 中，而不是 `register_db_mysqli.php` 或 `register_db_pdo.php`。
