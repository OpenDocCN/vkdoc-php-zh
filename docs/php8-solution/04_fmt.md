...
请注意，您需要在附加文本的开头创建一个空格，除非您希望两个字符串不间断地运行。这种简写被称为**组合串联运算符**，在组合许多字符串时非常有用，例如在构建电子邮件消息的内容或循环数据库搜索的结果时。
...
> **注意**
> `for`循环开始处括号内的第一个和第三个表达式可以包含多个用逗号分隔的语句。例如，循环可能使用两个独立递增或递减的计数器。
...
*   `public`: 这使得它在任何地方都是可见的，包括在类定义之外，允许你调用一个方法，访问或者改变一个属性的值，或者使用一个常量的值。
