...
消除重复和其他维护问题的过程称为*规范化*。表 9-1 包含了说明非常不良做法的项目。名字和姓氏应该在不同的列中。地址应该按照街道、城镇和邮政编码分成单独的列。这个过程被称为“原子性”这个相当笨拙的名字，因为像原子一样，数据被分成最小的成分。于是*原子*或*不可分*。
...
2.  从[出版社的页面下载第](http://apress.com) 9 章的文件。并将它们解压到新的`鸟类`文件夹中。
...
| 列名 | 类型 | 长度/值 | 默认 | 属性 | 空 | 索引 | A_I |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `reserves_id` | `INT` | `4` | `NULL` | `UNSIGNED` | `NOT NULL` | `PRIMARY` | `AUTO_INCREMENT` |
| `bird_pen` | `ENUM` | `'Yes','No'` | `NULL` |  | `NOT NULL` |  | `NO` |
| `entry_member` | `VARCHAR` | `60` | `NULL` |  | `NOT NULL` |  | `NO` |
| `entry_non_member` | `VARCHAR` | `60` | `NULL` |  | `NOT NULL` |  | `NO` |
| `location_id` | `INT` | `4` | `NULL` | `UNSIGNED` | `NOT NULL` | `INDEX` | `NO` |

### 重要的

遵循与前面 `location` 表中 `bird_id` 相同的过程，将 `location_id` 作为索引。
...
| `reserves_id` | `bird_pen` | `entry_member` | `entry_non_member` | `location_id` |
| --- | --- | --- | --- | --- |
| `1` | `Yes` | `Free` | `1` | `1` |
| `2` | `Yes` | `1` | `£2` | `2` |
| `3` | `Yes` | `Free` | `1` | `3` |
...
2.  单击 `location` 表中的 `location_id`，然后单击 `reserves_info` 表中的 `location_id`。如果您以错误的顺序执行此操作(即，如果您首先在 `reserves_info` 表中单击 `location_id`)，您将会看到一条错误消息。
...
主菜单上的按钮将使公众能够查看单个表和连接的表。图 9-10 显示了该菜单。菜单存储在*包含*文件夹中，清单 9-10b 给出代码。
...
<div class="container" style="margin-top:30px;border: 3px black solid;">
<!-- Header Section -->
<header class="jumbotron text-center row" id="includeheader" style="margin-bottom:2px; padding:20px; background-color:#CCFF99;">
    <?php include('includes/header.php'); ?>
</header>
<!-- Body Section -->
<div class="content mx-auto" id="contents">
...
清单 9-18 创建可打印表单 (`pay-with-check.php`)
