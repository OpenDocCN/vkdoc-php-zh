...
这是执行实际加密的调用。其参数含义明确，您可以在 `http://www.php.net/mcrypt` 找到关于 `mcrypt_encrypt()` 函数的更多详细信息。`MCRYPT_MODE_CBC` 指定了



“密码分组链接（cipher block chaining）”加密方法；该方法使用了一种链接机制，其中每个数据块的加密都依赖于前几个块的加密结果，但第一个块除外，它使用 `IV` 代替。
...
648XCH11.qxd 11/17/06 3:37 PM Page 381
