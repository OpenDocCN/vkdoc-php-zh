...
```php
$garden = new App\EmptyGarden(20, 30); // this is a pretty good size 20x30' garden
...
如果当你创建一个`EmptyGarden`的时候为你的宽度和高度传递一个字符串或者一个类或者负数会发生什么？
...
一个`PlotArea`告诉你在这个地块区域你有多少块地可以种植。一个圆形花园可能有 20 块地，半径为 10 英尺。一个长方形的花园可能有 40 块地，面积为 10×10 英尺。注意，它是一个接口，而不是一个具体的类。
