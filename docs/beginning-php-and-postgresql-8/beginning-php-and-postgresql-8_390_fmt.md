# 区分大小写，206

- 停在第一个匹配项

- 连接数组元素，217

- Perl 正则表达式修饰符，199

## 转换字符

- 停止命令，`pg_ctl` 程序，594

- 转换为括号表达式，198

- 转换为替换值，213

- 将换行符转换为 HTML，210

- `str_ireplace` 函数，219

- 将特殊字符转换为 HTML，211

- `str_pad` 函数，223

- `str_replace` 函数，219

- 转换为小写，208

- `str_word_count` 函数，225

- 转换为大写，208

- 将不常见字符转换为 HTML，210

- `strcasecmp` 函数，207

[www.it-ebooks.info](http://www.it-ebooks.info/)

**848**

■ 索引

## 计数

- 区分大小写，195

- 子串出现次数，221

- 模式是否存在，201

- 字符数量，224

- 将 HTML 转换为文本，213

- 单词数量，225

- 将文本转换为 HTML，212

## 委托字符串替换

- 过程，203

### 字符串函数

- `count_chars`，224

- 确定字符串长度，205

- `ereg`，195

- 分割字符串，204

- `ereg_replace`，196

- 基于分隔符，215

- `eregi`，196

- 不区分大小写，198

- `eregi_replace`，197

- 区分大小写，197

- `explode`，216

- 转换为子串数组，216

- `get_html_translation_table`，212

- 查找参数在字符串中的位置

- 不区分大小写，218

- `htmlentities`，210

- 区分大小写，217

- `htmlspecialchars`，211

- `implode`，217

- 参数最后出现的位置，218

- `join`，217

- 在特殊字符前插入反斜杠分隔符

- `ltrim`，222

- 特殊字符，202

- 第一个片段在/不在 `str2` 中的长度，207

- `nl2br`，210

- 将字符串填充到指定字符数，223

- `parse_str`，215

- 解析为各种变量，215

- `preg_grep`，201

- 移除字符

- `preg_match`，201

- 从开头移除，222

- `preg_match_all`，201

- 从结尾移除，223

- `preg_quote`，202

- HTML 和 PHP 标签，214

- `preg_replace`，203

- 替换模式

- `preg_replace_callback`，203

- 所有出现位置，203

- `preg_split`，204

- 不区分大小写，197

- `rtrim`，223

- 区分大小写，196

- `split`，197

- 替换字符串

- `spliti`，198

- 不区分大小写，219

- `sql_regcase`，198

- 区分大小写，219

- `str_ireplace`，219

- 字符串部分替换，222

- `str_pad`，223

- 返回字符串的剩余部分

- `str_replace`，219

- 在参数出现之后，219

- `str_word_count`，225

- 在两个参数之间，220

- `strcasecmp`，207

- 在数组中搜索模式，201

- `strcmp`，206

- 在字符串中搜索模式

- `strcspn`，207

- 所有出现位置，201

- `strip_tags`，214, 456, 528

- 不区分大小写，196

[www.it-ebooks.info](http://www.it-ebooks.info/)

■ 索引

**849**

- `strlen`，205

- `strrpos` 函数，218

- `strpos`，217

- `strspn` 函数，207

- `strrpos`，218

- `strstr` 函数，219

- `strspn`，207

- `strtok` 函数，215

- `strstr`，219

- `strtolower` 函数，208

- `strtok`，215

- `strtotime` 函数，284

- `strtolower`，208

- `strtoupper` 函数，208

- `strtotime`，284

- `strtr` 函数，213

- `strtoupper`，208

- `subclass`，面向对象编程，162

- `strtr`，213

- `subDays` 方法，294

- `substr`，220, 240

- `subject` 属性，消息，380, 384

- `substr_count`，221

- `subMonths` 方法，295

- `substr_replace`，222

- `subnet` 转换器，395–397

- `trim`，223

- `substr` 函数，220

- `ucfirst`，209

- 使用示例，240

- `ucwords`，209

- `substr_count` 函数，221

- 字符串函数，PostgreSQL，724

- `substr_replace` 函数，222

- 字符串处理，PHP 5，4

- `substring` 函数，PostgreSQL，724

- 字符串插值，75–77

- 减法 (-) 运算符，71

- 双引号，75

- `subWeeks` 方法，296

- heredoc 语法，77

- 单引号，76

- `subYears` 方法，297

- `sum` 函数，PostgreSQL，725

- 字符串操作，205–226

- 超全局变量，PHP，63, 67

- Perl 正则表达式元字符，199

- `$_COOKIE`，66

- Perl 正则表达式修饰符，199

- `$_ENV`，67

- 字符串偏移语法，PHP 5，191–192

- `$_FILES`，66

- 字符串运算符，71

- `$_GET`，65

- 字符串运算符，PostgreSQL，721

- `$_GLOBALS`，67

- 字符串解析，6

- `$_POST`，65

- 字符串

- `$_REQUEST`，67

- 本地化格式，280

- `$_SERVER`，65

- `strip_tags` 函数

- `$_SESSION`，67

- 清理用户数据，528

- 超级用户密码，PostgreSQL，650

- `stripos` 函数，218

- Smarty 模板引擎，456

- 超级用户，PostgreSQL

- 字符串操作，214

- 判断用户是否为超级用户，653

- `stripslashes` 函数，34

- 从源码安装 PostgreSQL，582

- 支持，PostgreSQL，576

- `strlen` 函数，205

- 代理键，750

- `strpos` 函数，217

[www.it-ebooks.info](http://www.it-ebooks.info/)

**850**

■ 索引

- swap meet 项目，767

- **T**

- 示例说明，768–771

- 制表符补全功能，`psql`，614

## 索引

`inserting data into tables`，768

`table schemas`，SQLite，548

`participant table`，767

`tables`，PostgreSQL

`purchase.php`，774

`altering table structure`，632

`trunk table`，767

`copying`，630

`using PHP`，773

`copying data from table to text file`，782

`switch statement`，PHP，81

`copying data to/from tables`，778–782

Sybase

`copying data from a table`，778

`PDO supported databases`，559

`exporting table OIDs`，780

符号链接

`creating`，629

`creating`，235

`creating table-formatted results`，697

`retrieving information about`，233

`creating temporary tables`，630

`retrieving target of`，235

`deleting`，632

`symlink function`，235


`foreign keys`，643

`syntax highlighting`

`getResultAsTable method`，696

`PHP configuration directives`，27

`naming conventions`，630

`syslog`

`referential integrity`，643

`define_syslog_variables directive`，40

`viewing list of tables`，631

`error messages in`，181

`viewing table structure`，631

`PHP configuration directives`，39

`tablespaces`，PostgreSQL，601–602

`syslog function`，182

`altering`，602

`syslog priority levels`，182

`creating`，601

`system commands`，252–253

`dropping`，602

`system function`，256

`owner`，601

`system level program execution`，254–258

`tabular date classes`

`backtick operator`，257

`Calendar package`，PEAR，286

`delimiting arguments`，255

`tabular output`

`escaping shell metacharacters`，255

`PostgreSQL database class`，689，695–697

`executing operating system level`

`paging`，689，701–704

`application`，256

`sorting`，689，699–701

`returning binary output`，257

`tags`

`executing shell commands`，257, 258

`strip_tags function`，Smarty，456

`outputting executed command’s`

`tar files`

`results.`，256

`installing PostgreSQL from source`，582

`sanitizing input`，254

`management of`，260

`system programs`

`tasks`，psql，613–619

`safe_mode_exec_dir directive`，518

`tcl option`

`installing PostgreSQL from source`，583

[www.it-ebooks.info](http://www.it-ebooks.info/)

**851**

`TCP sockets`，260

`time`

`template compilation`，Smarty，450

`Calendar package`，PEAR，285–288

`Coordinated Universal Time`，271

`template0`/`template1` databases，625

`localized formats`，280

`templates directory`，Smarty，451, 452

`max_execution_time directive`，519

`templates_c directory`，Smarty，451

`standardizing format for`，271

`templating engines`，447–449

`TIME datatype`，PostgreSQL，636

`benefits of`，448

`WITH TIME ZONE`，636

`delimiters`，448

`time functions`，PHP

`separating presentational from business`

*参见* `date and time functions`，PHP

`logic`，448

`time functions`，PostgreSQL，723, 724

`Smarty templating engine`，449–471

`time_limit parameter`，`ldap_search()`，404

`syntax of typical Smarty template`，449

`timeofday function`，PostgreSQL，724

`syntax of typical template`，448

`timeouts`

`TEMPORARY keyword`

`PDO_ATTR_TIMEOUT attribute`，560

`creating sequences`，633

`TIMESTAMP datatype`，PostgreSQL，637

`creating temporary tables`，630

`WITH TIME ZONE`，637

`temporary tables`，630

`timestamps`

`ternary (==) operator`，74

`retrieving file’s last access time`，238

`testing`

`retrieving file’s last changed time`，238

`facilitating unit tests`，260

`retrieving file’s last modification time`，239

`user bandwidth`，397–398

`setting file modification/access times`，253

`TEXT datatype`，PostgreSQL，640

`Unix timestamp`，271

`text files`

`Windows limitation`，276

`copying data from a text file`，779

`TLS (Transport Layer Security)`，402

`copying data from table to text file`，782

`tmp_name variable`

`error messages in`，181

`$_FILES array`，348

`text searching`

`TO_CHAR function`，PL/pgSQL，736

`indexes`，PostgreSQL，749

`TG_XYZ variables`

`toaddress attribute`，messages，381

`trigger functions`，PostgreSQL，745

`total space`

`this keyword`

`identifying on disk partition`，236

`accessing private fields`，139

`touch function`，253

`referring to fields`，OOP，137

`trace parameter`

`static fields`，153

`SoapClient constructor`，504

`throwing an exception`，183

`traces`，exception class methods

`ticks`，78

`getTrace`，186

`register_tick_function function`，78

`getTraceAsString`，186

`unregister_tick_function function`，78

`track_errors parameter`，31, 180

`tiers`，555

`transaction isolation`，766

[www.it-ebooks.info](http://www.it-ebooks.info/)

**852**

`transactions`，765–775

`languages supporting`，740

`ACID tests`，765

`modifying`，740

`atomicity`，765

`NEW`/`OLD` constructs`，740, 741, 742,

743, 745

`begintransaction method`，PHP，772

`commit method`，PHP，772

不同触发器的操作顺序，741

提交（committing），765

删除（removing），741

一致性（consistency），765

`CASCADE` 选项，741

定义，765–766

`RESTRICT` 选项，741

持久性（durability），766

规则与触发器，747


说明性示例, 768–771

触发器函数的特殊变量, 745

隔离性（isolation）, 766

`TG_XYZ` 变量, 745

`max_prepared_transactions` 设置,

查看现有触发器, 746

PostgreSQL, 597

编写触发器函数, 741–747

嵌套事务, 771

`trim` 函数, 223

PHP, 771–775

`TRUE` 状态

PHP 数据对象, 571

`BOOLEAN` 数据类型, 640

PostgreSQL, 766–771

`truncate` 函数, Smarty, 456

`rollback` 方法, PHP, 772

主干表（trunk table），swap meet 项目, 767

`rollbacktosavepoint` 方法, PHP, 772

信任认证方法

`pg_hba.conf` 文件, PostgreSQL, 655

回滚（rolling back）, 765, 771

保存点（savepoints）, 769

`setsavepoint` 方法, PHP, 772

`try ... catch` 块

捕获多个异常, 188

异常处理, 184

SQLite, 535

PHP 5 特性, 4

传输层安全（TLS）协议

`ldap_start_tls` 函数, 402

`tsearch2` 模块, 749

触发器

`ERROR: Can’t find tsearch config by`

变量函数, 99

区域设置, 757

触发器, PostgreSQL, 739–747

全文索引, PostgreSQL, 755–759

添加, 739

获取/安装 `tsearch2`, 755

`AFTER` 触发器, 740, 741

停用词（stopwords）, 758

`ALTER TRIGGER` 命令, 740

使用全文索引, 757

`BEFORE` 触发器, 740, 741

使用 `tsearch2` 工作, 756

`CREATE TRIGGER` 命令, 739

调优（tuning）

数据访问, 740

*参见* 性能调优, PostgreSQL

元组（tuples）

定义要执行的过程, 740

`PGSQL_TUPLES_OK` 值, 675

`DROP TRIGGER` 命令, 741

类型属性，消息, 383

函数参数, 741

类型转换，PHP

函数返回值类型, 741

数据类型, 54

与函数比较, 741

运算符, 54

[www.it-ebooks.info](http://www.it-ebooks.info/)

■索 引

**853**

类型转换，PHP

未知文件类型, 232

运算符和, 69

`unregister` 命令, `pg_ctl` 程序, 594

`TYPE` 字段, `pg_hba.conf` 文件, 654

`unregister_tick_function` 函数, 78

类型提示, 147

unseen 属性，消息, 381

类型标识符函数, 57

`unserialize_callback_func` 指令, 24

类型转换（type juggling）, 55

`unset` 函数, 434

类型相关函数, 56

更新数据

类型说明符

`ldap_modify` 函数, 417

`printf` 语句, 49

使视图可交互, 711

类型变量

`pg_update` 函数, 684

`$_FILES` 数组, 348

PostgreSQL, 681, 684

更新规则, 710

类型，PHP

`sqlite_changes` 函数, 546

全等运算符（`===`）, 73

升级，PostgreSQL, 609

类型化, 5

■

`UPLOAD_ERR_FORM_SIZE`, 350

**U**

`UPLOAD_ERR_INI_SIZE`, 350

`U` 选项, `psql`, 612

`UPLOAD_ERR_NO_FILE`, 351

`ucfirst` 函数, 209

`UPLOAD_ERR_OK`, 350

`ucwords` 函数, 209

`UPLOAD_ERR_PARTIAL`, 351

udate 属性，消息, 381

`upload_max_filesize` 参数, 38, 347, 355

UID（用户 ID）

`upload_tmp_dir` 参数, 38, 347

检索文件所有者的用户 ID, 240

上传（uploads）

uid 属性，消息, 384

文件上传，HTTP, 346–355

`umask` 函数, 241

文件上传，PHP, 345–346

`UNIQUE` 属性

`file_uploads` 指令, 38

PostgreSQL 数据类型, 644

`HTTP_Upload` 类, PEAR, 355–357

唯一索引，PostgreSQL, 750

`:upper:` 字符类, 195

唯一性，PostgreSQL 索引, 749

`upper` 函数, PostgreSQL, 724

主键索引, 750

URL 重写

单元测试, 260

在使用 cookie 时允许/限制, 428

定制 PHP 安装, 17

`referer_check` 指令, 430

下载 Apache, 9

检索会话名称, 426

下载 PHP, 11

`session.referer_check` 指令, 430

下载 PostgreSQL, 580

使用 SID 持久化, 426

安装 Apache/PHP, 11–13

`url_rewriter.tags` 指令, 432

安装 PEAR, 262

`use_trans_sid` 指令, 431

安装 PostgreSQL, 582–585

URLs

Unix 纪元, 272

一次性 URL, 342

Unix 时间戳, 271

用户友好 URL, 313–317

[www.it-ebooks.info](http://www.it-ebooks.info/)

**854**

■索 引

可用性，网站

userauth 表

导航提示, 313–323

*参见* 用户认证表

`use_cookies` 指令, 428

用户

`use_only_cookies` 指令, 428

自动登录，会话处理, 437

`ignore_user_abort` 指令, 27

usecatupd/useconfig/usecreatedb/usename

`PHP_AUTH_USER` 认证

列

变量, 327

`pg_shadow` 表, PostgreSQL, 653

## 索引

## 索引条目

`检索文件所有者的用户 ID`, 240

`用户账户`

`清理用户数据`, 524–528

`保护 PostgreSQL`, 650

### 用户, PostgreSQL

`用户认证表`
- `添加用户`, 658
- `基于数据库的认证`, 331
- `在所有表上授予权限`, 661
- `基于 IP 地址的认证`, 333
- `管理用户的权限`, 657
- `userauth 表`, 332
- `修改用户属性`, 658

`用户带宽`

`pg_shadow 表`, 652
- `测试`, 397–398
- `移除用户`, 658

### 用户自定义函数, PostgreSQL, 727–737

`usesuper 列`
- `创建`, 727
- `pg_shadow 表`, PostgreSQL, 653

`PL/pgSQL 函数`, 730–736

`usesysid 列`
- `安全性`, 728
- `pg_shadow 表`, PostgreSQL, 653

`SQL 函数`, 728

`usort 数组函数`, 123
- `函数类型`, 728

`UTC (协调世界时)`, 271

`USER 字段`, `pg_hba.conf 文件`, 654

### V

#### 用户友好的 URL

`VACUUM 命令`, PostgreSQL, 602–603
- `Apache 回环功能`, 314, 315–316
- `自动清理参数`, 604
- `导航提示`, 313–317
- `注意：手动清理`, 603

`PHP 代码`, 316
`VACUUM FREEZE 命令`, 603

### 用户界面选项类别

`VACUUM FULL 命令`, 603
- `安装 PostgreSQL`, 586

`VACUUM VERBOSE 命令`, 598, 603

### 用户登录管理, 337–344

`Validate_US 包`, PEAR, 226–227
- `密码指定`, 337–339
- `安装`, 226
- `密码可猜测性`, 339–342

`phoneNumber 方法`, 227
- `恢复/重置密码`, 342–344

`postalCode 方法`, 227

`region 方法`, 227

`user 参数`
- `pg_connect 函数`, 668
- `ssn 方法`, 227

### 用户注册

`使用`, 227
- `密码指定`, 337–339

### 验证类

`user_agent 参数`, 38
- `Calendar 包`, PEAR, 286

`user_dir 参数`, 37, 520

[www.it-ebooks.info](http://www.it-ebooks.info/)

**索引**

**855**

### 值赋值

`视图`, PostgreSQL, 707–708
- `变量声明`, PHP, 59
- `创建视图`, 707

### 值

`删除视图`, 708
- `ldap_compare 函数`, 411
- `使视图可交互`, 711–716

`ldap_get_values 函数`, 406
- `使用 PHP 查询视图`, 716

`ldap_get_values_len 函数`, 406
- `从 PHP 使用视图`, 716–717

`ldap_mod_del 函数`, 418

`VOLATILE 函数`
- `ldap_sort 函数`, 411
- `用户自定义函数`, PostgreSQL, 728

`valuntil 列`

### W

`pg_shadow 表`, PostgreSQL, 653

`w3schools 网站`, 304

### 网页表单/页面

`var 参数`
- `insert 标签`, Smarty, 463
- `自动选择表单数据`, 310–311

`VARCHAR 数据类型`, PostgreSQL, 640
- `显示修改日期`, 283

##### 变量声明

`示例`, 304–305
- `PL/pgSQL 函数`, 731
- `在线表单教程`, 304

`RECORD 类型`, 731
- `用 PHP 生成表单`, 308–310

### 变量函数, 99

`在脚本间传递数据`, 304
- `安全风险`, 100
- `将表单数据传递给函数`, 306

### 变量修饰符, Smarty, 454–457

`将 PHP 变量传递到 JavaScript`

### 变量, PHP, 58–67

`函数`, 311–313
- `认证变量`, 327–328

`PHP 与 Web 表单`, 303–313

### 使用多值表单

`超全局变量`, 63–67

### 组件, 307–308

`变量声明`, 58–60

### Web 服务, 473–514

`显式声明`, 59
- `高知名度部署`, 475

`引用赋值`, 59
- `MagpieRSS`, 479–486

`值赋值`, 59
- `NuSOAP`, 492–502

### 变量作用域, 60–63

`使用 Web 服务`, 494–495
- `函数参数`, 61
- `创建方法代理`, 495–496

`全局变量`, 61
- `调试工具`, 501

`局部变量`, 60
- `指定 HTTP 代理`, 501

`静态变量`, 62
- `错误处理`, 500–501

### 可变变量, 67

`生成 WSDL 文档`, 499–500

`variables_order 参数`, 33
- `发布 Web 服务`, 496–498

### VersionMismatch 错误

`返回数组`, 498–499

`faultstring 属性`, NuSOAP, 500
- `安全连接`, 502

### 版本

`简易资讯聚合 (RSS)`, 473, 476–486
- `PDO_ATTR_CLIENT_VERSION`
- `选用原因`, 474
- `PDO_ATTR_SERVER_VERSION`

[www.it-ebooks.info](http://www.it-ebooks.info/)

**856**

**索引**

`SimpleXML`, 474, 486–491

### `with-pgsql` 选项

`SOAP`, 474, 491–512
- `启用 PostgreSQL 扩展`, 665

`PHP 5 的 SOAP 扩展`, 502–512

### `with-python/with-tcl` 选项

`支持`, PHP 5, 4
- `从源码安装 PostgreSQL`, 583

### 使用 C# 客户端与 PHP Web 服务交互, 512–514

### 单词

`统计字符串中的单词数`, 225

### `work_mem` 设置, PostgreSQL, 596

### 封装器

`WHERE 子句`, PostgreSQL
- `fopen 封装器`, 38
- `索引`, 759
- `流封装器`, 390
- `部分索引`, 753, 754

### 可写文件

`WHILE 循环`, PL/pgSQL, 732
- `检查文件是否可写`, 241

`while 语句`, PHP, 81

### 预写日志, PostgreSQL

`WhitePages.com`
- `checkpoint_segments` 设置, 599
- `PostgreSQL 用户`, 577
- `checkpoint_timeout` 设置, 599

### 空白字符

`WSDL (Web 服务描述语言)`
- `Perl 正则表达式修饰符`, 199
- `创建 SOAP 服务器`, 506
- `预定义字符范围`, 195
- `配置指令`, 507

### 宽度说明符

`NuSOAP 特性`, 493
- `printf 语句`, 49
- `生成 WSDL 文档`, 499–500

##### Windows

`获取`, 494
- `自定义 PHP 安装`, 17

### `wsdl` 参数

`下载 Apache`, 10
- `SoapClient 构造方法`, 503

`下载 PHP`, 11
- `SoapServer 构造方法`, 508

`下载 PostgreSQL`, 580–581

### `wsdl_cache_dir` 配置指令

`安装 Apache/PHP`, 13–16
- `创建 SOAP 服务器`, 508

`安装 PEAR`, 263

### `wsdl_cache_enabled` 配置指令

`安装 PostgreSQL`
- `在 2000/XP/2003 上`, 585–589

`wsdl_cache_ttl` 配置指令, 508
- `在 95/98/ME 上`, 589

### X

`X 选项`, `psql`, 612

### `:xdigit:` 字符类, 195

### XML

`GNOME XML 库`, 503
- `SimpleXML`, 486–491

### `with-docdir/without-docdir` 选项

`asXML 方法`, 489
- `从源码安装 PostgreSQL`, 583

`SOAP 定义`, 491

### `with-perl/with-pgport` 选项

`支持`, PHP 5, 4
- `从源码安装 PostgreSQL`, 583

[www.it-ebooks.info](http://www.it-ebooks.info/)

**索引**

**857**

`XML_Parser` 包, PEAR, 261

### ZIP 代码

`XML_RPC` 包, PEAR, 261
- `使用 Validate_US 包`, PEAR, 227

`XML-RPC 协议`, 261

### zip 文件, 260

`XOR 运算符`, 73

### `zlib.output_compression` 参数, 24

`xpath 方法`, SimpleXML, 490

### `zlib.output_handler` 参数, 24

### Y

### `zlib-devel` 包

`y2k_compliance` 参数, 23
- `从源码安装 PostgreSQL`, 583

`Zmievski, Andrei`, 449

### Z

`Zend 脚本引擎`, 2

### `zend.ze1_compatibility_mode` 指令, 22

[www.it-ebooks.info](http://www.it-ebooks.info/)