...
因为问题永远不会单独显示——意思是在房间的上下文之外——`get_title()`方法只返回`NULL`。记住它不需要声明，因为它是抽象父类的一部分。
...
通过将以下代码添加到`Question_Model`来实现该方法：
...
更新投票计数很简单：对于给定的问题 ID，`vote_question()`方法将投票计数增加`1`。将这个方法添加到`Question_Model`类：
...
最后，为了将问题标记为已回答，具有给定 ID 的问题的`is_answered`列被更新为`1`。将以下代码添加到`Question_Model`:
...
使用第七章的中的房间标记，通过将以下代码添加到位于`system/views/room.inc.php` : 的新文件中来创建房间视图
...
将代码添加到`class.room_model.inc.php`:
...
关闭一个房间与打开一个房间的过程相同，只是在`rooms`表中`is_active`列被设置为`0`。将以下代码插入`Room_Model`:
...
使用添加到`class.room_model.inc.php`中的以下代码所示的查询完成上述步骤:
...
将以下代码添加到`Room`类的构造函数中，以添加对这四个动作的支持:
