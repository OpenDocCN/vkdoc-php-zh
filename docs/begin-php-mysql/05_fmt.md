...
与`asort()`类似，`arsort()`维护键/值的相关性。但是，它以相反的顺序对数组进行排序。其原型如下:
...
PHP 手册提供了一个很好的例子，展示了“自然”排序数组的含义。考虑以下几项:`picture1.jpg`、`picture2.jpg`、`picture10.jpg`、`picture20.jpg`。使用典型算法对这些项目进行排序会产生以下顺序:
...
`array_slice()`函数根据起始值`offset`和`length`返回数组的一部分。其原型如下:
