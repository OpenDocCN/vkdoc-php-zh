...
这里一个方便的做法是为复杂查询命名，这样它就可以存储在数据库中，并由外部应用程序以统一的方式访问。这正是视图的作用。在 PostgreSQL 中，视图被定义为给定查询的存储表示形式。一旦定义，视图在许多方面可以被视为一个虚拟表。虽然视图本身不存储数据，但它可以像任何其他表一样被查询，您甚至可以基于其他视图创建视图。
