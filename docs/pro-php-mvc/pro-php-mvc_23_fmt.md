...
大多数关系型 SQL 查询是 `INSERT`、`UPDATE`、`DELETE` 或 `SELECT` 查询。我们将创建方法来处理这些类型的查询，从 `SELECT` 查询开始，如清单 9-9 所示。
...
构建 `INSERT`、`UPDATE` 和 `DELETE` 查询的方法比 `_buildSelect()` 方法简单一些。请看清单 9-10。
