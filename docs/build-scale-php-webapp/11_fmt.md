...
1.  修改 `/var/lib/pgsql/data/postgresql.conf` 并设置 `listen_addresses='*'`。我们不需要让专门将其设置为私有 IP 地址，因为这是我们唯一的 IP 地址，除非我们将 GCP 配置为允许，否则无法从互联网访问它。用 `systemctl restart postgresql` 重启 PostgreSQL，这样修改就会生效。
...
![img/487291_1_En_11_Figc_HTML.jpg](img/487291_1_En_11_Figc_HTML.jpg)
