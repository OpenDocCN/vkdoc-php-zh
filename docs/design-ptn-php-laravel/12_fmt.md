...
你需要一个真正的怪物。我知道没有怪物、圣诞老人或者怪物圣诞老人 <sup>2</sup> 这样的东西，但怪物存在于电子游戏中。让我们选一个最笨的怪物:僵尸。经典僵尸没那么可怕。你可以从僵尸身边走开，它永远也抓不到你。流行文化电影和游戏，如《Z 世界大战》和《死路四条》，已经将这些原本可笑、缓慢、愚蠢的怪物变成了快速、强大的[泰坦](http://en.wikipedia.org/wiki/Attack_on_Titan) <sup>3</sup> 。我看不出我们有什么理由不加入僵尸的行列！
...
```php
$monster = new App\ExtremelyFast($monster);
$monster = new App\ExtremelyFast($monster);
$monster = new App\ExtremelySmart($monster);
...
```php
$monster->jumpAttack(); // no such method - errors
```
...
在本例中，您将使用一个名为[`robclancy/presenter`](https://github.com/robclancy/presenter) <sup>7</sup>的演示者。我已经冒昧地将它添加到了`composer.json`中，所以请确保您运行了`composer update`，然后您就可以使用`php artisan serve`运行 web 服务器了。
...
@if ($user->favoriteColor)
  <span style="background-color: {{ $user->favoriteColor }}">Hello
there!</span>
@else
...
```php
<span {{ $user->favoriteColorStyle }}>Hello there!</span>
```
...
1.  设计模式:可重用面向对象软件的元素，第 196 页
2.  [`http://en.wikipedia.org/wiki/Santa's_Slay`](http://en.wikipedia.org/wiki/Santa%27s_Slay)
3.  [`http://en.wikipedia.org/wiki/Attack_on_Titan`](http://en.wikipedia.org/wiki/Attack_on_Titan)
4.  [`http://php.net/manual/en/language.oop5.magic.php`](http://php.net/manual/en/language.oop5.magic.php)
5.  [`http://php.net/manual/en/language.oop5.overloading.php#object.call`](http://php.net/manual/en/language.oop5.overloading.php#object.call)
6.  [`http://php.net/manual/en/language.oop5.overloading.php#object.get`](http://php.net/manual/en/language.oop5.overloading.php#object.get)
7.  [`https://github.com/robclancy/presenter`](https://github.com/robclancy/presenter)
8.
