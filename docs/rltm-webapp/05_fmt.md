...
![image](img/sq.jpg) **注** MVC 可以追溯到 1979 年 <sup> 2 </sup> 从它的概念提出到现在的 30 多年里，已经被数十种语言的程序员解释、再解释、再解释。实现 MVC 没有绝对正确的方法。这是一个指导发展的哲学原则，而且，和所有的哲学一样，它在不同的阵营中会有不同的实践。不要为此感到紧张。
...
为了保持数据库整洁并消除重复信息，需要对数据库进行规范化。 <sup>3</sup>
...
第一个表`presenters`，存储演示者的姓名、电子邮件和唯一的 ID(见表 5-7 )。
...
`rooms`表将存储房间的唯一 ID、名称以及是否处于活动状态(参见表 5-8 )。
...
`questions`表将存储问题的唯一 ID、问题所属的房间、问题文本以及问题是否已被回答(参见表 5-9 )。
...
`question` _ `votes`表将存储问题的 ID 和当前的票数(见表 5-10 )。

表 5-10 。`question_votes`表
...
`Room_owners`表将存储房间和主持人的 id(参见表 5-11 )。

表 5-11 。`room_owners`表
...
<sup>1</sup> 关于适应性布局的图库，参见`http://mediaqueri.es`。
