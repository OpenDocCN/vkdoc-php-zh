...
3.  接下来，设置`$curPage`的值。您稍后将创建的导航链接将通过查询字符串传递所需页面的值，因此您需要检查`curPage`是否在`$_GET`数组中。如果是，就使用那个值，但是要确保它是一个整数，在它前面加上`(int)`转换操作符。否则，将当前页面设置为 0。在上一步中的代码后立即插入以下代码:
...
4.  现在，您已经获得了计算起始行和构建 SQL 查询以检索记录子集所需的所有信息。在前面步骤中的代码后立即添加以下代码:
...
5.  但是有一个问题。`$curPage`的值来自查询字符串。如果有人在浏览器地址栏中手动更改数字，`$startRow`可能会大于数据库中的记录数。如果`$startRow`的值超过`$totalPix`，则需要将`$startRow`和`$curPage`都重置为`0`。将此条件语句添加到上一步中的代码之后:
...
7.  保存页面并将其重新加载到浏览器中。你应该只看到六个缩略图，而不是八个，如图 14-5 所示。

    更改`SHOWMAX`的值以查看不同数量的缩略图。

8.  缩略图网格上方的文本不会更新，因为它仍然是硬编码的，所以让我们来解决这个问题。在页面正文中找到以下代码行:
