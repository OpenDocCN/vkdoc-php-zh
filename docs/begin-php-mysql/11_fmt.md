...
PECL 是一个用 C 编写的 PHP 扩展库。与用 PHP 编写的相同功能相比，用 C 编写的扩展通常能提供更好的性能。这些扩展通常是现有 C 库的包装，用于向 PHP 开发人员公开这些库的功能。托管在 `https://pecl.php.net` 的 PECL 扩展是常用的扩展，但也可以在 GitHub 上找到扩展，如这个示例所示，演示了如何下载、编译和安装第三方 PHP 扩展。
