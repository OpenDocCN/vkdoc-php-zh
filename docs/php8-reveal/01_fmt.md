...
1.  **在标准网络应用中没有实质性的性能提升**
    ...
    Stogov 已经声明使用 JIT 为开发人员提供了一个独特的机会，可以进入 PHP 还不是主要参与者的领域。他设想 PHP 被用于非 Web、CPU 密集型的场景，在这些场景中，性能直接受所用语言的能力影响，RUST 或 Python 是首选。对 Stogov 和他的团队来说，PHP 8 是采取必要措施用 JIT 增强 PHP 的正确时机。为此，他们选择了为 LuaJit 创建的动态汇编程序(DynAsm)。DynAsm 目前支持 POSIX 平台和 Windows 上的 x86、x86_64 CPUs 以及 ARM、ARM 64、MIPS、MIPS64 和 PPC。这使得 PHP 的 JIT 能够被当今部署 PHP 的最流行的平台所支持。
