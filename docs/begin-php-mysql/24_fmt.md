...
要在交互模式下使用 MySQL，需要先进入界面。如前所述，您可以通过传递适当的凭证来实现这一点。在前一个例子的基础上，假设您想要与驻留在您的开发环境中的 `dev_corporate_com` 数据库进行交互:
...
*   `kill id[,id2[,idN]]`: 终止`id`、`id2`到`idN`指定的进程。您可以使用`processlist`命令查看进程号。
