...
| `ReflectionFunction` | 函数（如 PHP 核心函数或用户函数） |
| `ReflectionProperty` | 属性 |
...
类常量允许您在每个类的基础上定义这样的值；它们不会在类的执行个体之间变更。从该类创建的所有对象都具有相同的类常量值。
...
“转发”调用是由`parent::`、`static::`引入的静态调用或由函数`forward_static_call()`调用的调用。
...
对`C::test()`的调用被完全解决，因此 C 类最初被存储为最后一个非转发调用。

函数 C 中没有`test()`方法，所以调用被隐式转发给它的父类。所以类 B 中的`test()`方法被调用。

#### 对 `A::foo()` 的调用

`test()`中的第一个调用专门将类 A 命名为作用域。这意味着调用已完全解决。被存储为最后一个非转发调用的类被更改为 A。
...
PHP 将任何名称以两个下划线为前缀的方法视为魔术的方法。PHP 在对象生命周期的特定时间“神奇地”调用这些方法（无需您调用它们）。我喜欢把它们想象成类似于在事件中被调用的钩子。当对象发生相关事件时，PHP 调用魔术方法。
...
1. [`http://www.php-fig.org/psr/psr-4/examples/`](http://www.php-fig.org/psr/psr-4/examples/)
2. [`https://php.net/manual/en/function.spl-autoload-register.php`](https://php.net/manual/en/function.spl-autoload-register.php)
3. [`https://www.owasp.org/index.php/PHP_Object_Injection`](https://www.owasp.org/index.php/PHP_Object_Injection)
4. [`https://en.wikipedia.org/wiki/Dependency_injection`](https://en.wikipedia.org/wiki/Dependency_injection)
5. [`https://php.net/manual/en/class.reflectionclass.php`](https://php.net/manual/en/class.reflectionclass.php)
6. [`https://php.net/manual/en/book.spl.php`](https://php.net/manual/en/book.spl.php)
```