...
`_partial()` 处理程序接受一个 URL（`$address`），该 URL 可以是绝对路径（例如，`www.google.com`）或相对于同一应用程序的路径（例如，`localhost/messages/feed.html`）。如果 URL 不以 `http` 开头，则将其视为相对 URL 并进行相应调整。我们向该 URL 发出 GET 请求，并将结果返回到模板的 `$_text` 数组中，该数组将渲染到最终的模板输出中。
