...
当控制器需要尝试自动渲染视图时，它会先渲染当前动作的视图，然后再渲染布局的视图。动作视图和布局视图都是`View`实例，但动作视图的标记会被插入到布局视图标记的正中央。现在，我们来看看**列表 13-7**中负责渲染视图的方法。
...
我们需要修改的最后一个`Controller`方法是`__construct()`方法，如**列表 13-8**所示。
...
为了帮助你理解新`Controller`类的完整结构，我在**列表 13-9**中包含了完整代码。
