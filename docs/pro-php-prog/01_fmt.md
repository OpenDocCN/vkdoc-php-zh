...
使用关键字`private`和`protected`可以更好地保护成员。这两个关键字的确切含义将在下一节解释。同样值得注意的是`public`是默认的可见性。如果没有指定成员或方法的可见性，则默认为`public`。
...
**清单 1-10。** 任何实现接口`Iterator`的类都可以在`for`循环中使用
