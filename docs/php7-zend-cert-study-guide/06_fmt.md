...
按位运算符`&`比较两个数的位位置。如果两个数在位置集合中都有一位，那么结果就是`true`。因此，`E_ALL & ∼E_NOTICE`已经设置了所有的位，除了那个表示`E_NOTICE`开启的位。
...
根据惯例，URL `http://my.host/cgi-bin/php?/etc/passwd` 会尝试将`/etc/passwd`传递给 PHP 二进制文件。通常，CGI 解释器打开并执行命令行中第一个参数指定的文件。
