...
在`dateZoom`插件中，您可以通过插入以下代码使您的方法可链接:
...
您可以通过将以下代码插入到`dateZoom`插件中来创建`zoom`方法:
...
在这个方法中，您将使用`.animate()`、`.dequeue()`和`.clearQueue()`方法来制作对象的动画并防止动画队列堆积；添加以下代码来实现这一点:
