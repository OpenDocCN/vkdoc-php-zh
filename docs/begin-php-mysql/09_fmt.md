...
将匹配字符串中`f`后面跟 0 或多个`o`的任何部分。这将匹配前面示例中的**食物**、**傻瓜**和 **fo4** 以及**快速**和**精细**等。这两个限定词对字符的重复次数都没有上限。如下例所示，可以添加这样的上限:
...
如果指定了可选输入参数 `limit`，则只返回该 `limit` 个数的子字符串。考虑一个例子:
...
您可以使用可选的`start`参数来定义字符串中的起始位置，而不是默认的 0 偏移量。可选的`length`参数可用于定义将用于比较的`str1`字符串的长度。

### 计算两个字符串之间的差

`strcspn()`函数返回一个字符串的第一段长度，该字符串包含在另一个字符串中找不到的字符。可选的`start`和`length`参数的行为方式与之前介绍的`strspn()`功能中使用的方式相同。其原型如下:
...
请注意，如果`O'Malley`不小心被写成`O'Malley`，`ucwords()`不会捕捉到错误，因为它认为一个单词被定义为一串字符，通过两边的空格与字符串中的其他实体分开。
...
函数`stripos()`的操作与`strpos()`相同，只是它不区分大小写地执行其搜索。
...
可选参数`offset`决定了`strrpos()`开始搜索的位置。假设您想要削减冗长的新闻摘要，截断摘要并用省略号替换被截断的部分。然而，不是简单地在期望的长度上显式地截断摘要，而是希望它以用户友好的方式操作，在最接近截断长度的单词末尾截断。这个函数非常适合这样的任务。考虑这个例子:
...
函数`str_replace()`区分大小写地将一个字符串的所有实例替换为另一个。其原型如下:

```php
mixed str_replace(string occurrence, mixed replacement, mixed str [, int count])
```

如果在`str`中找不到`occurrence`，则原始字符串被不加修改地返回。如果定义了可选参数`count`，则只有在`str`中发现的`count`个事件会被替换。
...
此示例结合使用`strstr()`函数和`ltrim()`函数来检索电子邮件地址的域名:
...
如果没有指定可选的`length`参数，则认为子串是从`start`开始到`str`结束的字符串。使用此功能时，请记住四点:

*   如果`start`为正数，返回的字符串将从字符串的`start`位置开始。
*   如果`start`为负，返回的字符串将从字符串的`length - start`位置开始。
*   如果提供了`length`并且是正数，返回的字符串将由`start`和`start + length`之间的字符组成。如果这个距离超过了字符串的总长度，则只返回从`start`到结束之间的字符串。
...
*   如果`start`为负，`replacement`将从`str`的`length - start`位置开始。
