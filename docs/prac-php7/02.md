...
与页眉类似，页面的主体部分定义了一行。一行有三列。左栏(col-sm-2)为导航菜单保留了两个单元格。菜单将以“药丸”格式显示(见图 2-3 ),并且对于不同的设备尺寸是灵活的。一条 PHP 语句导入 nav.php 的*文件，该文件包含链接的实际代码清单(参见清单 2-3b )。*
...
旧模板和新索引文件之间代码的唯一区别是新的头引用。清单 2-7 中的代码片段显示了这些变化。完整的代码包含在 index.php 的*文件中，可以从[网站下载。com](http://apress.com) 网站。*
