...
使用表 10-1 中的属性，并为将接受排序规则的列选择排序规则 `utf8_general_ci` (`reg_date` 和 `member_level` 没有排序规则)。创建列时，将出现 `member_id` 和 `user_name` 索引的弹出对话框。对于 `member_id` 弹出窗口，这两个字段都将是主字段。在用户名索引弹出窗口中，在第一个字段中输入 `user_name`，在第二个字段中输入 `UNIQUE`。
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `member_id` | INT | 8 | 无 | UNSIGNED | 否 | PRIMARY | 是 |
| `user_name` | VARCHAR | 12 | 无 | | 否 | UNIQUE | 是 |
| `email` | VARCHAR | 60 | 无 | | 否 | | 是 |
| `password` | CHAR | 60 | 无 | | 否 | | 是 |
| `reg_date` | DATETIME | | 无 | | 否 | | 是 |
| `member_level` | TINYINT | 2 | 无 | | 否 | | 是 |
...
| 列名 | 类型 | 长度/值 | 默认 | 属性 | 空 | 索引 | A_I |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `post_id` | INT | 8 | 无 | UNSIGNED | 否 | PRIMARY | 是 |
| `user_name` | VARCHAR | 12 | 无 | | 否 | | 是 |
| `subject` | VARCHAR | 60 | 无 | | 否 | | 是 |
| `message` | TEXT | | 无 | | 否 | FULLTEXT | 是 |
| `post_date` | DATETIME | | 无 | | 否 | | 是 |
...
<nav class="float-left navbar navbar-expand-xl navbar-trans navbar-light">
    ...
    <div class="btn-group btn-group-md collapse navbar-collapse navbar" id="collapsibleMenu" role="group" aria-label="Button Group">
        ...
        switch ($menu) {
            case 1: //index.php
        ?>
                <button type="button" style="width: 110px;" class="btn btn-secondary bg-primary disabled" onclick="location.href = ''">Home Page</button>
                <button type="button" style="width: 110px;" class="btn btn-secondary bg-primary disabled" onclick="location.href = ''">Login</button>
                <button type="button" style="width: 110px;" class="btn btn-secondary bg-primary" onclick="location.href = 'logout.php?name=Logout'">Logout</button>
                <button type="button" style="width: 110px;" class="btn btn-secondary bg-primary disabled" onclick="location.href = ''">Register</button>
                <button type="button" style="width: 120px;" class="btn btn-secondary bg-primary" onclick="location.href = 'view_posts.php?name=Your Quotes'">Your Quotes</button>
                <button type="button" style="width: 120px;" class="btn btn-secondary bg-primary" onclick="location.href = 'post.php?name=Add A Quote'">Add A Quote</button>
                <button type="button" style="width: 120px;" class="btn btn-secondary bg-primary" onclick="location.href = 'forum_c.php?name=Comic Quotes'">Comic Quotes</button>
                <button type="button" style="width: 120px;" class="btn btn-secondary bg-primary" onclick="location.href = 'forum_w.php?name=Wise Quotes'">Wise Quotes</button>
                <button type="button" style="width: 120px;" class="btn btn-secondary bg-primary" onclick="location.href = 'search.php?name=Search Quotes'">Search Quotes</button>
        <?php
        break;
        ?>
    </div>
...
索引页面代码的顶部(清单 10-2a )包含了前一段。该段通过搜索`member_id`值来确定显示哪个菜单。如果存在，则用户已经登录。变量`$menu`设置为 1，这将激活除登录和注册之外的所有按钮。如果`member_id`值不存在，则用户尚未登录。变量`$menu`设置为 5。案例 5 的菜单块(在`header.php`文件的*中)仅激活登录和注册按钮。*
...
Listing 10-3b Checking the Registration Page (`process-register-page.php`)
...
名为`process-login.php`的文件启动登录过程。清单 10-5b 中给出了代码。
...
Listing 10-5b Creating the Code for Processing the Login (`process_login.php`)
...
| 粉红色的百合 | 滑稽的引语 | "打高尔夫球是一种很好的散步方式。"马克·吐温 |
| 粉红色的百合 | 滑稽的引语 | "生活是一件接一件糟糕的事情。"马克·吐温 |
...
| 神话之王 | 滑稽的引语 | "许多小事情因为正确的广告而变得很大."马克·吐温 |
| 神话之王 | 明智的报价 | "同时做两件事等于什么都不做。"Publilius Syrus |
| 巨人台阶 12 | 明智的报价 | "从未犯过错误的人从未尝试过新事物."阿尔伯特·爱因斯坦 |
...
清单 10-8 中给出的代码只从数据库 `forum` 表中选择滑稽的引文。
...
该查询只选择主题为“Wise Quotes”的记录，报价将按发布的升序排序，即最早的先排序。
...
Listing 10-10 创建一个显示个人成员帖子的页面 (`view_posts.php`)。
