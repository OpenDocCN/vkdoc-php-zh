...
DELETE FROM product_category WHERE product_id = inProductId; DELETE FROM product WHERE product_id = inProductId;
...
```sql
-- 创建 catalog_remove_product_from_category 函数 CREATE FUNCTION catalog_remove_product_from_category(INTEGER, INTEGER) RETURNS SMALLINT LANGUAGE plpgsql AS $$
