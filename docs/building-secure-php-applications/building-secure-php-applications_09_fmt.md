...
![图 2-7 – 展示 Cookie 与会话使用的请求-响应周期](img/609480_1_En_2_Fig7_HTML.jpg)
...
5.  **SameSite 属性**：`samesite` 属性允许你定义浏览器应如何处理跨站请求中的 Cookie。它有助于防御 CSRF（跨站请求伪造）攻击。使用 `Strict` 作为值可确保 Cookie 仅在第一方请求中发送，从而增强安全性。
...
3.  **身份验证与授权**：会话数据通常用于跟踪用户的身份验证状态并确定其在应用程序中的访问权限。保护会话数据对于维护安全的用户会话至关重要。
