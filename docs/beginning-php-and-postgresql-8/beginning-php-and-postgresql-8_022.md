...
• **面向对象支持**：版本 4 引入了一定程度的面向对象功能，尽管它被认为是一个比较普通的实现。尽管如此，这些新特性在吸引习惯于使用传统面向对象编程语言的用户方面发挥了重要作用。标准类和对象开发方法得以提供，此外还有对象重载和运行时类信息。更全面的`OOP`实现已在版本 5 中提供，并在第 5 章中介绍。

[www.it-ebooks.info](http://www.it-ebooks.info/)

C H A P T E R 1 ■ A N I N T R O D U C T I O N T O P H P

**3**

• **原生会话处理支持**：版本 3.X 用户通过第三方包`PHPLIB`（http://phplib.sourceforge.net）可用的`HTTP`会话处理，在版本 4 中原生集成。该特性为开发者提供了一种手段，以前所未有的效率和简便性跟踪用户活动和偏好。第 15 章介绍了`PHP`的会话处理能力。

• **加密**：`MCrypt`（http://mcrypt.sourceforge.net）库被纳入默认发行版，为用户提供使用`Blowfish`、`MD5`、`SHA1`和`TripleDES`等加密算法的完整加密和哈希加密。第 18 章深入探讨了`PHP`的加密能力。

• **ISAPI 支持**：`ISAPI`支持使用户能够将`PHP`与微软的`IIS` Web 服务器结合作为`ISAPI`模块使用，显著提高了其性能和安全性。

• **原生 COM/DCOM 支持**：对 Windows 用户的另一个好处是`PHP 4`能够访问和实例化`COM`对象。这一功能打开了与 Windows 应用程序广泛的互操作性。

• **原生 Java 支持**：在提升`PHP`互操作性的另一项举措中，从`PHP`应用程序绑定到`Java`对象的支持在版本 4.0 中提供。

• **Perl 兼容正则表达式（PCRE）库**：`Perl`语言长期以来被誉为字符串解析领域的王者。开发者知道强大的正则表达式功能将在`PHP`的广泛接受中发挥重要作用，因此选择直接整合`Perl`的功能而非重新实现，将`PCRE`库包纳入`PHP`的默认发行版（从版本 4.2.0 开始）。第 9 章详细介绍了这一重要特性，并对通常令人困惑的正则表达式语法进行了概述。

除了这些特性外，版本 4 还添加了数百个函数，极大地增强了该语言的能力。在本书中，将讨论这些功能的很大一部分，因为它们在版本 5 发布中仍然同样重要。

##### 缺点

`PHP 4`代表了该语言成熟度的一次巨大飞跃。新版本提供的新功能、强大能力和可扩展性吸引了大量新兴和专家级开发者，使其稳固地跻身于 Web 脚本巨擘之列。然而，在语言业务中维持用户的喜爱是一项艰巨的任务；程序员通常持有一种“你最近为我做了什么？”的心态。`PHP`开发团队牢记这一点，因为不久之后他们又着手进行另一项艰巨的任务，一项可能使该语言成为 Web 脚本世界 800 磅大猩猩的任务：`PHP 5`。

#### PHP 5

版本 5 是`PHP`语言发展史上的又一个分水岭。尽管之前的主要版本拥有大量新的库添加，但版本 5 对现有功能进行了改进，并添加了通常与成熟编程语言架构相关的若干特性：
...
• **大幅改进的面向对象能力**：对`PHP`面向对象架构的改进是版本 5 最显著的特性。版本 5 包含众多功能添加，例如显式构造函数和析构函数、对象克隆、类抽象、变量作用域、接口，以及`PHP`在处理对象管理方面的重大改进。第 6 章和第 7 章对此主题进行了全面介绍。

• **Try/Catch 异常处理**：在结构化编程语言中设计自定义错误处理策略，讽刺的是，容易出错且不一致。为解决此问题，版本 5 现在支持异常处理。作为许多语言（包括`C++`、`C#`、`Python`和`Java`）中错误管理的主流方式，异常处理为标准化错误报告逻辑提供了极佳的手段。这种新的便捷方法将在第 8 章介绍。

• **改进的字符串处理**：早期版本的`PHP`默认将字符串视为数组，这种做法表明了该语言传统的对数据类型松散的态度。这一策略在版本 5 中得到了调整，引入了专用的字符串偏移语法，并且之前的做法已被弃用。新特性、变化以及新语法带来的影响将在第 9 章讨论。

• **改进的 XML 和 Web 服务支持**：`XML`支持现在基于`libxml2`库，并引入了一个用于解析和操作`XML`的、颇具前景的新扩展，称为`SimpleXML`。此外，`SOAP`扩展现已可用。在第 20 章中，将介绍这两个新扩展，以及一些优秀的第三方 Web 服务扩展。

• **原生支持 SQLite**：始终热衷于提供选择的开发者们，为强大而紧凑的`SQLite`数据库服务器（http://www.sqlite.org/）添加了支持。`SQLite`为那些寻求一些重量级数据库产品中的许多功能、但又不希望承担相应管理开销的开发者提供了一个便捷的解决方案。`PHP`对该强大数据库引擎的支持将在第 22 章介绍。

版本 5 还提供了许多其他改进和添加，本书将在相关章节中酌情介绍。

随着版本 5 的发布，`PHP`的普及率达到历史最高点。截至本书付印时，`PHP`已安装在近 1900 万个域名上（Netcraft，http://www.netcraft.com/）。根据 E-Soft, Inc.（http://www.securityspace.com/）的数据，`PHP`是迄今为止最流行的 Apache 模块，在几乎 54%的 Apache 安装中可用。

到目前为止，本章仅讨论了该语言的特定版本特性。每个版本都共享一组共同的特征，这些特征在吸引和保留大量用户方面发挥着非常重要的作用。在下一节中，您将了解这些基础特性。


好的，作为高级文档工程师和翻译员，我将严格遵循您的注意事项和示例格式，对给定英文文本进行翻译。以下是译文：
...
早期对 PHP 5 的介绍提到了一个事实：与之前的版本相比，新版本更侧重于质的提升而非量的增加。此前的主要版本都伴随着 PHP 默认库的巨大扩充，每次发布都增加了数百个新函数。目前，有 113 个库可用，其中总共包含超过 1,000 个函数。尽管您可能知道 PHP 能够与数据库交互、处理表单信息以及动态创建页面，但您可能不知道 PHP 还可以执行以下操作：
...
当你在多用户环境中（例如 ISP 的共享服务器上）部署 PHP 时，你可能希望限制其功能。可以想象，让所有用户完全掌控所有 PHP 功能可能会引发对服务器资源和文件的利用或破坏。作为在共享服务器上使用 PHP 的安全措施，PHP 可以在受限模式（即*安全模式*）下运行。
...
####### `safe_mode_protected_env_vars`（字符串）
