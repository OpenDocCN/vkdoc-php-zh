...
```
$query = "SELECT productid, name, price FROM product ORDER BY name"; $result = pg_query($query);
echo "Query Type: ".pg_result_status($result, PGSQL_STATUS_STRING)."<br />"; echo "Query Status: ".pg_result_status($result, PGSQL_STATUS_LONG)."<br />";
```
...
`<a href="addtocart.php?productid=VALUE">Add to Cart</a>';`当然，这也意味着查询中必须包含一个标识键。产品表的主键是`productid`。此外，此键应放在查询的首位，因为脚本需要知道用哪个字段值来替换`VALUE`。
