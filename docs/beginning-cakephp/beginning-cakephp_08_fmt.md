...
你可以通过编辑`app/config/routes.php`文件。在这种情况下，最好先按照约定构建数据库，然后进入`routes.php`文件，创建一些指向相应控制器的 URL 别名。你可以在数据库中创建一个名为`posts`的表来存储所有博客文章，并在`posts_controller.php`文件中编写一个动作，从 URL 中提取日期并为检索到的文章渲染视图。然后，在`routes.php`中，你可以为所有以`/view`开头的 URL 字符串编写一个路由，使其指向`Posts`控制器并传递日期参数。如果现在这听起来不太明白，别担心——你将在第 10 章中更详细地了解 Cake 的路由可能性。只需确保在考虑如何命名数据库表以及相应的控制器和模型时，首先坚持约定。之后，你可以使用路由来确保 URL 的结构符合你的喜好。
