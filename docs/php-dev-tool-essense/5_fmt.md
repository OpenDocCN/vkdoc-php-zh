...
*   `language`–这是你的模块的翻译文件所在的地方。框架应用使用 `ZendI18n` 模块，并使用 `.po` 文件来提供文本翻译。

*   `src`–这是绝大多数模块代码所在的地方。该文件夹包含构成模块的控制器、窗体、服务和其他应用逻辑。
...
一旦安装成功，你应该在新创建的`symphony_demo`文件夹中看到一些目录(图 5-5 )。
