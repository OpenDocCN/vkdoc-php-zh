...
1.  在页面主体的第一个表格单元格中，将`$row['created']`更改为`$row['date_created']`，以匹配 SQL 查询中的别名。
...
表 16-3 PHP 日期和时间相关的类和函数

| 类别 | 名字 | 参数 | 描述 |
| :--- | :--- | :--- | :--- |
| 班级 | `DateTime` | 日期字符串，`DateTimeZone`对象 | 创建一个区分时区的对象，包含可用于日期和时间计算的日期和/或时间信息。 |
...
| 功能 | `time()` | 没有人 | 为当前日期和时间生成 Unix 时间戳。 |
...
| 单位 | `DateTime`/`date()` | `strftime()` | 描述 | 例子 |
| --- | --- | --- | --- | --- |
...
### 表 16-7 DateInterval 类使用的 ISO 8601 时段指示符
