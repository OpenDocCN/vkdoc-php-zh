...
PHP 不是我们可以使用的唯一语言，因为有许多语言可以让我们构建 web 应用。许多语言也让我们使用实时网络技术，但有些语言做得比其他语言好。Node.js 与实时 web 技术紧密相关，主要是因为它的事件性质和 socket.io， [<sup>4</sup>](#Fn4) ，这可能是最著名的实时 web 框架；Ruby 有很多解决方案，最流行的是 FAYE[??](#Fn5)。NET 有一个相当新的微软支持的解决方案，叫做 SignalR [<sup>6</sup>](#Fn6) Python 有很多基于 Tornado 框架的解决方案； [<sup>7</sup>](#Fn7) 以此类推。
