...
[`www.dolphin-gallery.co.uk`](https://www.dolphin-gallery.co.uk)
...
2.  从[出版社的页面下载第](http://apress.com) 11 章的 paypalcart 文件。并将它们解压到你新的 `paypalcart` 文件夹中。
...
Listing 11-1 The PayPal Cart Home Page (`index.php`)
...
可以从 Apress 网站下载并查看`found_paintings.php`文件的 HTML 代码。它类似于您之前看到的代码。清单 11-2 显示了显示找到的画作的 PHP 代码。
...
我们还在商店的 `found_paintings` 页面的标题中放置了一个 PayPal 查看购物车按钮(位于 `header.php`)。查看购物车按钮的代码将类似于清单 11-3b 。如前所述，此代码的格式可以由贝宝更改。经常去 PayPal 网站下载最新的格式。
...
`admin_add_paints.php` 站点的 HTML 代码类似于本书中显示的其他表单代码。因此，此处不包含。但是，您可以从 Apress 网站下载并在您的文本编辑器中查看它。表单被提交到 PHP 文件 `admin_add_paints.php`。清单 11-4 提供了这个代码。
...
Listing 11-4 Code for Adding Paintings (`admin_add_painting.php`)
```
...
`thumb`的验证使用`filter_var`属性`FILTER_VALIDATE_URL`来确保艺术品图片的位置格式正确。
...
2.  从[出版社的页面下载第](http://apress.com) 11 章的定制购物车文件。并将它们解压缩到新的`customcart`文件夹中。
...
这只是一个指向名为`forgot.php`的页面的链接。清单 11-8 显示的是`forget.php`的代码。
...
如果用户点击“忘记密码？”链接，他们将看到如图 11-8 所示的屏幕。
...
`forgot.php`文件的 HTML 代码类似于前面显示的其他代码。因此，此处不包含。你可以从 Apress 网站下载。然而，让我们看看`process_forget.php`页面的 PHP 代码，如清单 11-8 所示。
...
自定义购物车代码和 PayPal 代码之间的唯一区别是购物车提供的链接。在自定义购物车代码中，链接被附加到`added.php`程序。在贝宝程序中，它会转到贝宝网站。这个定制的购物车链接还会将`art_id`传递给`added.php`。
...
检查所选绘画的细节以确定显示两个消息中的哪一个，或者“一幅绘画已被添加”或者“那些绘画中的另一幅已被添加”。会话变量`cart`包含一个带有`id`、`quantity`和`price`索引的三维数组。通俗地说，购物车包括由 ID(购买了什么)、数量(多少)和价格(每个)定义的每个订单。如果用户已经选择购买该画，则数量值增加(`quantity++`)(这由`if`语句确定)。
