...
清单 5-1 Service Provider’s `register()` Method
...
Listing 5-2 The Default Configuration for a Redis Connection, Located in `app/config/database.php`
```
...
在`boot()`方法中，您可以包含任何依赖于其他配置、服务或系统其他方面的逻辑。它在服务容器被 Laravel 实例化之后运行，因此，它被允许使用其他服务提供者提供的任何其他功能或服务容器中存在的东西。
...
Listing 5-3 Example boot() Method in a Service Provider
```
...
Listing 5-4 Controller with Dependencies Injected That the Service Container Can Resolve by Itself
```
...
Listing 5-5 Service Requiring a Dependency on a ClaimRepository Class That Can Automatically Be Resolved Using Reflection
...
图 5-1 一种可能的面向对象设计方法是使用抽象来解决拥有多种通知类型的需求
...
Listing 5-6 A Possible Usage of the Design in Figure 5-1 as Done by Hand, Not Using the Service Container or Dependency Injection
...
Listing 5-7 Example Service Container Binding for the Aforementioned Notification System
```
...
Listing 5-8 The Service Container Configuration Needed to Support All Possible Repositories in the Application
```
...
Listing 5-9 An Example of What Not to Do...Rely on a Concrete Method
```
...
**Listing 5-11** An Example Laravel Job That Accepts `$patient` and `$provider` Objects, Then Modifies the Patient’s Set Provider to Be the One Passed into the Handle Method
